...
if (isset($_POST["insert"])) {
    ...
    if ($con->query($sql)) {
        ...
        if ($es_id != '') {
            ...
            header("location:exam_set_choose_exam_insert.php?exam_set=$es_id");
        } else {
            $_SESSION['successinsert'] = "สำร็จ";
            header("location:exam_form_insert.php?checkteach=$ck_id");
        }
    }
}
if (isset($_POST["update"])) {
    ...
    if ($con->query($sql)) {
        ...
        header("location:exam_form_update.php?exam=$e_id");
    }
}
if (isset($_POST["delete"])) {
    $e_id = $_POST["delete"];

    $sql1 = "DELETE from tb_exam where e_id = '$e_id'";
...
if (isset($_POST['subject_u'])) {
    $subject_u = $_POST['subject_u'];
    if ($subject_u != '') {
        $subjectsql = " and ck.sj_id = '$subject_u'";
    } else {
        $subjectsql = "";
    }
    $output = '';
    ...
    while ($r1 = $rs1->fetch_object()) {
        $output .= '
        <option value="' . $r1->u_id . '">หน่วยที่ ' . $r1->u_unit . ' ' . $r1->u_name . '</option>
        ';
    }
    echo $output;
}